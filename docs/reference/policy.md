...
`maida assert` and `maida run` resolve the policy in this order:
...
The repeated gate and the existing metric checks live in the same `assert:` block, so there is one place to tune confidence, CI cost, and acceptable behavior:
...
| YAML key | Type | Default | `maida run` flag | Description |
|---|---|---|---|---|
| `trials` | `int` | `3` | `--trials` | Fresh isolated agent invocations per gate |
| `confidence_level` | `float` | `0.95` | `--confidence-level` | Confidence level for the two-sided Wilson interval |
| `pass_rate_threshold` | `float` | `0.90` | `--pass-rate-threshold` | Minimum underlying pass rate required by the statistical decision |

`trials` must be at least 1, `confidence_level` must be strictly between 0 and 1, and `pass_rate_threshold` must be between 0 and 1. Invalid values fail before the agent script runs and name the invalid field. More trials narrow the interval but multiply agent runtime and model cost.

At `trials: 1`, the verdict preserves binary behavior. The current cost-conscious `trials: 3` default uses an explicit compatibility rule: unanimous outcomes decide PASS or FAIL, while mixed outcomes use the Wilson interval and may be FAIL or INCONCLUSIVE. The machine report records the applied `decision_rule`; this rule is not a claim that 3/3 successes prove a 90% pass rate at 95% confidence.
