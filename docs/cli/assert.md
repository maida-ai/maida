...
Evaluates one already-completed trace against v2 invariant and measured rules, or standalone CLI flags. Statistical, distributional, and plan metrics require `maida run` or `maida drift`; this single-run interface rejects them instead of silently skipping checks.
...
| `--policy` | `.maida/policy.yaml` (auto-detected) | Policy file with an explicit supported version (`2` or `2.1`) |
...
**Exit codes:** `0` all checks passed; `1` one or more checks failed; `2` invalid policy, run or baseline not found; `10` internal error.
