...
def _traces_request(*, session_id: str = "trace-session", duration_ms: int = 20) -> ExportTraceServiceRequest:
...
def test_trace_capture_persists_source_shape(temp_data_dir):
    ...
    records = [json.loads(line) for line in (capture_dir / "spans.jsonl").read_text().splitlines()]
    assert len(records) == 1
...
def test_complete_batch_is_validated_before_any_write(temp_data_dir):
    ...
    response = _post(TestClient(create_claude_code_app(load_config())), "/v1/logs", request)
...
def test_rejects_wrong_content_type_oversized_and_invalid_ids(temp_data_dir):
    ...
    response = _post(TestClient(create_claude_code_app(load_config())), "/v1/traces", request)
    assert response.status_code == 400
    assert not (temp_data_dir / "captures").exists()
