...
def test_markdown_preview_link_becomes_absolute_with_raw_true() -> None:
    ...
    assert _HOOK.relative_preview_links(content, BASE) == (
        "![Guardrails demo](https://github.com/maida-ai/maida/blob/v0.5.0/docs/assets/guardrails.gif?raw=True)"
    )
...
def test_markdown_non_preview_link_becomes_absolute_without_raw() -> None:
    ...
    assert _HOOK.relative_non_preview_links(content, BASE) == (
        "See [docs/guardrails.md](https://github.com/maida-ai/maida/blob/v0.5.0/docs/guardrails.md) for details."
    )


def test_nested_badge_markdown_link_rewrites_outer_relative_target() -> None:
    content = "[![License](https://img.shields.io/badge/license-Apache--2.0-green)](LICENSE)"
    assert _HOOK.relative_non_preview_links(content, BASE) == (
...
def test_html_preview_src_becomes_absolute_with_raw_true() -> None:
    ...
    assert _HOOK.relative_html_preview_links(content, BASE) == (
        '<img alt="demo" src="https://github.com/maida-ai/maida/blob/v0.5.0/docs/assets/timeline.gif?raw=True">'
    )
...
def test_preview_preserves_fragment_and_appends_raw_after_query() -> None:
    ...
    assert _HOOK.relative_preview_links(content, BASE) == (
        "![shot](https://github.com/maida-ai/maida/blob/v0.5.0/docs/assets/shot.gif?v=1&raw=True#top)"
    )
...
def test_rewrite_readme_links_handles_markdown_and_html_together() -> None:
    ...
    assert "https://github.com/maida-ai/maida/blob/v0.5.0/docs/assets/demo.gif?raw=True" in rewritten
    assert "https://github.com/maida-ai/maida/blob/v0.5.0/docs/reference/policy.md#overview" in rewritten
    assert 'src="https://github.com/maida-ai/maida/blob/v0.5.0/docs/assets/other.gif?raw=True"' in rewritten
...
def test_readme_base_url_uses_main_for_dev_versions() -> None:
    assert _HOOK.readme_base_url("0.5.0.dev3") == ("https://github.com/maida-ai/maida/blob/main/")
    assert _HOOK.readme_base_url("0.5.0") == ("https://github.com/maida-ai/maida/blob/v0.5.0/")
