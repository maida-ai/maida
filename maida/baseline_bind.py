...
def validate_policy_against_baseline(policy: AssertionPolicy, baseline: dict[str, Any] | None) -> None:
    ...
    for name, metric in policy.metrics.items():
        ...
        if metric.kind is MetricKind.MEASURED:
            if policy.source_format == "v2" and metric.limit is None and not baseline_values(baseline or {}, name):
